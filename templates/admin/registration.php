<div class="wrap">
    ...
    <table class="wp-list-table widefat fixed striped rfmp_page_registration">
        ...
        <?php foreach ($fields as $row) { ?>
            <tr>
                ...
                <?php if ($row->type == 'checkbox') { ?>
                    <?php if ($row->value == '') { ?>
                        <td class="value column-value"><?php echo __('Yes', 'mollie-forms');?></td>
                    <?php } else { ?>
        ...
        <tr>
            ...
            <td class="value column-value"><?php echo $this->helpers->getCurrencySymbol($registration->currency ?: 'EUR') . ' ' . number_format($registration->total_price, $this->helpers->getCurrencies($registration->currency ?: 'EUR'), ',', '');?></td>
        </tr>
        <tr>
        ...
        <table class="wp-list-table widefat fixed striped rfmp_page_registration_subscriptions">
            ...
            foreach ($priceOptions as $priceOption)
            {
                ...
                <tr>
                    ...
                    <td><?php echo $this->helpers->getCurrencySymbol($priceOption->currency ?: 'EUR') . ' ' . number_format($priceOption->price, $this->helpers->getCurrencies($currency), ',', '') . ' ' . $frequency;?></td>
                    <td><?php echo $this->helpers->getCurrencySymbol($priceOption->currency ?: 'EUR') . ' ' . number_format($optionVat, $this->helpers->getCurrencies($currency), ',', '');?></td>
                    <td><?php echo $this->helpers->getCurrencySymbol($priceOption->currency ?: 'EUR') . ' ' . number_format($optionPrice, $this->helpers->getCurrencies($currency), ',', '') . ' ' . $frequency;?></td>
                </tr>
            <?php } ?>
            </tbody>
            <tfoot>
                <tr>
                    <td colspan="3"><strong><?php echo __('Total', 'mollie-forms');?></strong></td>
                    <td><strong><?php echo $this->helpers->getCurrencySymbol($currency ?: 'EUR') . ' ' . number_format($totalVat, $this->helpers->getCurrencies($currency), ',', '');?></strong></td>
                    <td><strong><?php echo $this->helpers->getCurrencySymbol($currency ?: 'EUR') . ' ' . number_format($totalPrice, $this->helpers->getCurrencies($currency), ',', '');?></strong></td>
                </tr>
            </tfoot>
        </table><br>
    <?php } ?>
    ...
    <table class="wp-list-table widefat fixed striped rfmp_page_registration_payments">
        ...
        foreach ($payments as $payment) {
            ...
            try {
                $mollie_payment = $mollie->get('payments/' . $payment->payment_id);
            } catch(Exception $e) {
            ...
            <tr>
                ...
                <td class="column-amount"><?php echo $this->helpers->getCurrencySymbol($payment->currency ?: 'EUR') . ' ' . number_format($payment->amount, $this->helpers->getCurrencies($payment->currency ?: 'EUR'), ',', '');?></td>

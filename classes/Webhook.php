...
class Webhook
{
    ...
    private $db, $mollieForms, $helpers, $email;
    ...
    private function handleRequest($query)
    {
        try {
            ...
            if (isset($query->query_vars['sub'])) {
	            ...
	            // send emails
	            $this->email->send($post, $status, $registration->id, $payment, 'customer');
	            $this->email->send($post, $status, $registration->id, $payment, 'merchant');
            ...
            // send emails
            $this->email->send($post, $status, $regPayment->registration_id, $payment, 'customer');
            $this->email->send($post, $status, $regPayment->registration_id, $payment, 'merchant');
